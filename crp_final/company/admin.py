...
class CompanyMembershipAdmin(admin.ModelAdmin):
    ...
    class CompanyAccountingSettingsAdmin(admin.ModelAdmin):
        list_display = (
            ...
            'updated_by_user')
        # All fields will be editable by default. Use fieldsets for organization.
        readonly_fields = ('created_at', 'updated_at', 'updated_by')

        list_select_related = (
            'company', 'updated_by', 'default_retained_earnings_account', 'default_accounts_receivable_control')
        search_fields = ('company__name',)
        # Raw ID fields are good for ForeignKey selection to Account
        raw_id_fields = (
        ...
        fieldsets = (
            (None, {'fields': ('company',)}),  # Company is PK, usually not editable after creation here
        ...
        def updated_by_user(self, obj):
            return obj.updated_by.name if obj.updated_by else "N/A"

        def formfield_for_foreignkey(self, db_field, request, **kwargs):
            # Filter Account choices to belong to the Company of this CompanyAccountingSettings instance
            # This instance is `self.model_instance` if editing, or not available if adding.
            # For adding, company FK on CompanyAccountingSettings must be selected first.
            ...
            # If adding new CompanyAccountingSettings, 'company' field on its form needs to be selected.
            # This filtering applies once that company is known.
            if not company_for_filtering and request.method == 'POST':  # Try from POST if adding
        ...
        def get_readonly_fields(self, request, obj=None):
            if obj:  # Editing an existing object
                return ('company',) + self.readonly_fields  # Add 'company' to existing readonly_fields
