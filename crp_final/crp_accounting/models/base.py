...
class TenantSafeDeleteManager(SafeDeleteManager, OriginalCompanyManager): pass


class UnfilteredTenantSafeDeleteManager(SafeDeleteManager, OriginalUnfilteredCompanyManager): pass


class TenantDeletedManager(SafeDeleteDeletedManager, OriginalCompanyManager): pass


class UnfilteredTenantDeletedManager(SafeDeleteDeletedManager, OriginalUnfilteredCompanyManager): pass


class TenantAllIncludingDeletedManager(SafeDeleteAllManager, OriginalCompanyManager): pass


class UnfilteredTenantAllIncludingDeletedManager(SafeDeleteAllManager, OriginalUnfilteredCompanyManager): pass
...
class TenantScopedModel(SafeDeleteModel):
    ...
    def save(self, *args, **kwargs):
        ...
        if self.company_id:
            company_instance_for_check = self.company if hasattr(self,
                                                                 'company') and self.company_id == self.company.pk else \
                (current_company_from_context if is_new and current_company_from_context else None)
        ...
        # *** START OF FIX for force_soft issue ***
        kwargs_for_super_save = kwargs.copy()
        kwargs_for_super_save.pop('force_soft', None)  # Remove force_soft if it exists
        # *** END OF FIX for force_soft issue ***

        # Perform full_clean only if update_fields is not used, using the modified kwargs
        if not kwargs_for_super_save.get('update_fields'):  # CHECK USING MODIFIED KWARGS
            excluded_fields = []
            if is_new and self.company_id:
                excluded_fields.append('company')

            # Call model's specific _set_derived_fields before cleaning, if it exists
        ...
        super().save(*args, **kwargs_for_super_save)  # PASS MODIFIED KWARGS TO SUPER

    def clean(self):
        super().clean()
        # Original clean logic from your file was:
        ...
        # This seems fine. Any model-specific clean should go in that model's clean().

    @classmethod
    def create_for_company(cls, company: Company, created_by_user: settings.AUTH_USER_MODEL,
                           **kwargs):  # Added created_by_user
        ...
        if not isinstance(created_by_user, get_user_model()):  # Check user type
        ...
        kwargs.pop('created_by_id', None)  # Pop user fields
        kwargs.pop('updated_by', None);
        kwargs.pop('updated_by_id', None)

        instance = cls(company=company, created_by=created_by_user, updated_by=created_by_user, **kwargs)
        instance.save()
        logger.info(
            f"Created new {cls.__name__} (ID: {instance.pk}) for Company '{company.name}' by {created_by_user.name}.")
        return instance
    ...
    def can_be_edited_by_user(self, user) -> bool:
        ...
        if not CompanyMembership:
            logger.error(f"Permission check failed: CompanyMembership model not available.")
            return False
        try:
    ...
    def __str__(self):
        ...
        if display_name: return f"{display_name} (Co: {company_prefix})"
        return f"{self.__class__.__name__} (ID: {self.pk}, Co: {company_prefix})"


# =============================================================================
# ExchangeRate Model (Tenant-Aware or Global) - Assuming this is correct as per your last version
# =============================================================================
class ExchangeRate(models.Model):
    ...
    date = models.DateField(_("Effective Date"), db_index=True,
                            help_text=_("The date this exchange rate is effective from (inclusive)."))
    rate = models.DecimalField(_("Exchange Rate"), max_digits=20, decimal_places=10,
                               help_text=_("1 unit of 'From Currency' = 'Rate' units of 'To Currency'."))
    source = models.CharField(_("Rate Source (Optional)"), max_length=100, blank=True, null=True,
    ...
    def clean(self):
        ...
        if self.rate <= Decimal('0'):  # Ensure rate is positive
